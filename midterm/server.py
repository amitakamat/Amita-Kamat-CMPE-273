...
class WalletServicer(wallet_pb2.WalletServicer):
    ...
    def encrypt(self, request, context):
        ...
        f = Fernet(self.key)
        return wallet_pb2.CardEncryptResponse(token=f.encrypt(str(request.card)))
    ...
    def decrypt(self, request, context):
        ...
        # TODO
        f = Fernet(self.key)
        return wallet_pb2.CardDecryptResponse(card_in_plain_text=f.decrypt(str(request.token)))
